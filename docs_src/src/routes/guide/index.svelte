<script context="module">
	export async function preload() {
		const sections = await this.fetch(`guide.json`).then(r => r.json()).catch(e => console.log(e));
		return { sections };
	}
</script>
...
<div bind:this={container} class='content linkify listify'>
	...
	<section data-id={section.slug}>
		<h2>
			...
			<small>
				<a href='https://github.com/halfnelson/svelte-native/edit/master/docs_src/content/guide/{section.file}' title='edit this section'>
					<Icon name='edit' /></a>
			</small>
		</h2>
		{@html section.html}
